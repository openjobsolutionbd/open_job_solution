...
## ১৮. Current Affairs সেকশন — সিঙ্ক-ভিত্তিক ইন্টিগ্রেশন

চতুর্থ সেকশন (`current-affairs/`) বাকি তিনটার (bcs-mcq, primary-mcq, written-exam) থেকে গঠনগতভাবে আলাদা — এটা এই রিপোতে সরাসরি এডিট করা হয় **না**।

- **উৎস:** এই ফোল্ডারের কনটেন্ট আসে `openjobsolutionbd/open_current_affairs` নামের সম্পূর্ণ আলাদা, স্বতন্ত্র রিপো থেকে, স্বয়ংক্রিয়ভাবে। `open_current_affairs` রিপোই এই কনটেন্টের একমাত্র "আসল" (source of truth) — এই রিপোতে না।
- **সিঙ্ক নিয়ম:** প্রতি ৩ দিন পরপর একবার, `open_current_affairs`-এর একটা GitHub Action এই ফোল্ডার (`current-affairs/`) হালনাগাদ করে। `open_current_affairs`-এ যত ইচ্ছা ঘন ঘন কাজ হোক, তার প্রভাব শুধু নির্ধারিত সিঙ্ক-মুহূর্তেই এই রিপোতে আসে।
- **⚠️ এডিট করার নিয়ম:** এই ফোল্ডারের ভেতরে কেউ (মানুষ বা AI) সরাসরি এডিট করলে, পরের সিঙ্কে সেটা মুছে গিয়ে উৎস-রিপোর কনটেন্ট দিয়ে প্রতিস্থাপিত হবে। **তাই `current-affairs/`-এ সরাসরি এডিট করা নিষিদ্ধ। কোনো পরিবর্তন করতে হলে তা `open_current_affairs` রিপোতেই করতে হবে।**
- **সীমানা:** সিঙ্ক-ব্যবস্থা টেকনিক্যালিভাবে `current-affairs/`-এর বাইরে (bcs-mcq/, primary-mcq/, written-exam/, বা root) কোনো ফাইল ছুঁতে পারে না — GitHub Action-এর trigger নিজেই `current-affairs/**` পাথে সীমাবদ্ধ (path-filtered)।
- **PWA পরিচয় (Section-এর "একটাই PWA" নিয়মের সাথে সংযোগ):** সিঙ্কের সময় স্বয়ংক্রিয়ভাবে এই সেকশনের নিজস্ব `<link rel="manifest">` লাইন বাদ দেওয়া হয়, যাতে এটা আলাদা ইনস্টলযোগ্য অ্যাপ হিসেবে জাহির না করে — root-এর `manifest.json`-ই একমাত্র উৎস থাকে।
- **Service Worker স্কোপ:** `current-affairs/docs/sw.js`-এর জন্য root `_headers`-এ `Service-Worker-Allowed: /current-affairs/docs/` এন্ট্রি আছে, বাকি সেকশনের প্যাটার্ন অনুসরণ করে।
- **ঠিকানা:** এই সেকশনের ঠিকানা `current-affairs/docs/` (অন্য সেকশনের মতো `current-affairs/` না) — কারণ উৎস রিপোর নিজস্ব ফোল্ডার-কাঠামোতে ওয়েবসাইট `docs/`-এর ভেতরে থাকে।
- **নিরাপত্তা-ব্যবস্থা:** প্রতি সিঙ্কে ফাইল-সংখ্যা যাচাই হয়, ব্যর্থ হলে স্পষ্ট error বার্তা তৈরি হয়, আর ব্যর্থতার ক্ষেত্রেও আগের কনটেন্ট অক্ষত থাকে (কখনো ভাঙা/খালি পাতা দেখাবে না)। বিস্তারিত: `open_current_affairs` রিপোর `.github/workflows/sync-to-job-solution.yml`।
